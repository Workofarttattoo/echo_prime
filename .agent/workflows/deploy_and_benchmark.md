...
```bash
nohup python $HOME/echo_prime/dashboard_server.py > dashboard.log 2>&1 &
```

2. Run the hive‑mind demo
```bash
python $HOME/echo_prime/demo_hive_mind.py
```
...
python3 $HOME/echo_prime/full_benchmark_runner.py --datasets gsm8k --samples 5

# Or run full suite (requires powerful hardware)
# bash $HOME/echo_prime/execute_full_benchmarks.sh
```
